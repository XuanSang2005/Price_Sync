...
- Done — step 11c (2026-07-08): claim/process split into two separate `@Transactional` methods on new bean `BatchProcessor` — `claimNext(owner)` (short tx: `findNextToClaim` + `markProcessing`) and `validateBatch(id)` (own tx: loop `validate` → `markValid`/`setAside`, then verdict). `WorkDispatcher.poll()` is NON-transactional and calls both through the proxy in sequence, so the claim tx commits before validation starts — ADR-04 short-claim restored (resolves the 11b tech debt). ADR-07 verdict: `failRate = (double) setAside/(valid+setAside)`; `failRate > 0.2 || valid == 0` → `markFail()` (status FAILED via dirty checking), else stays PROCESSING for Mapper. Verified per-record both ways: ABORT (2/4 set aside = 50% → FAILED) and PASS (batch 33 `verdict-pass-01`: `vp-4` DELETE no-price → VALID/exempt, `vp-5` -999 → SET_ASIDE/INVALID_PRICE; 4 VALID + 1 SET_ASIDE = 20% → PROCESSING).
- Validator bug caught by the evidence rule (2026-07-08): the DELETE price-exemption was inverted TWICE — first `changeType != "DELETE"` (String reference compare → always true → dead exemption), then after switching to the `ChangeType` enum the negation was dropped (`...equals(DELETE)` instead of `!...equals(DELETE)`), so DELETE was wrongly required to have a price while NEW/UPDATE with a bad price slipped through. The aggregate log "4 VALID, 1 SET_ASIDE" was IDENTICAL under correct vs inverted logic (DELETE and the -999 record merely swapped verdicts) — only the per-record psql table exposed it. Lessons: compare enums with `==`/`!=`, not `.equals`+String; a correct aggregate count can hide a swapped-record bug — always inspect per-record; also tightened the price rule `< 0` → `<= 0` (reject 0.00).
- Next: Mapper — config-driven field mapping of VALID records toward the MNT file (ask the user to propose the design first). After: `XstoreMntBuilder` (FHEAD/FDETL/FDELE/FTAIL streamed to a temp file), `XcenterWriter`, Retry Scheduler (exp backoff), record-level supersede (ADR-06 level 2), security filter chain (API key/HMAC/IP allowlist), console endpoints, tests.
