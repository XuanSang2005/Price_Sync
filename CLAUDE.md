...
- Next: step 9b — claim `RECEIVED` batch via native `@Query` `FOR UPDATE SKIP LOCKED` (`findNextToClaim()` returning `Optional<PriceBatch>`), `markProcessing()` domain method on entity, `@Transactional` poll, dirty checking (already taught, user has the full lesson — let them implement from it); then 2-instance concurrency experiment. After: lease columns + reaper (ADR-04), record-level idempotency/supersede, Validator, Mapper, MNT Builder, XcenterWriter, Retry Scheduler, security filter chain, console endpoints.
