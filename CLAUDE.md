...
- Next: Validator chapter (claimed batch → per-record validation per FR-03, set-aside with reasons, PARTIAL/abort >20% per ADR-07) — user proposes design first. After: Mapper, MNT Builder, XcenterWriter, Retry Scheduler, record-level supersede, security filter chain, console endpoints.
